...
def visualize_country_forecast(country_dfs, country_name, active_measures_override=None):
    ...
    if active_measures_override is not None and set(active_measures) != set(active_measures_override):
        _, _, weekly_x_axis, weekly_new_cases, active_measures = forecast_for_country(country_dfs, country_name,
                                                                                      active_measures_override=active_measures_override)
...
def create_toggle_buttons(country_dfs, country_name):
    ...
    for measure in sorted(country_df.columns):
        if measure == 'Date' or (measure.endswith(
                '3') and not measure == 'Testing_3') or measure == 'Awareness Campaigns_2' or measure == 'Public Transport Closure_1':  # exclude measures that were never performed
            continue
